...
export async function GET(request: NextRequest) {
  try {
    ...
    // Test if we can insert a test donation (this will fail due to foreign key constraints, but we can see the error)
    const testDonation = {
      patron_id: user.id,
      listing_id: 'test-listing-id', // This will fail foreign key constraint
    ...
    console.error('Debug donations table error:', error);
    return NextResponse.json(
      { 
        error: 'Debug failed',
